...
const Preview = ({ meta}) => {
  const { name, percent, status, previews } = meta
  return (
    <span style={{ alignSelf: 'flex-start', margin: '10px 3%', fontFamily: 'Helvetica' }}>
      {previews},{name}, {Math.round(percent)}%, {status}
    </span>
  )
}

const NoDropzoneLayout = ({ previews, submitButton, input, files, dropzoneProps, name }) => {
...
multiple={false}
styles={{inputLabelWithFiles:{display:'none'}}}
