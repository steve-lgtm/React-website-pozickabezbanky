...
export default function Ziadost() {
  ...
  const scriptUrl = "https://script.google.com/macros/s/AKfycbzI69xGdz3Qnr-IInHExOFJy53Z2E8pZNysrIgMPGlZHNj8vNMlCXsVBDFpDai60BmISg/exec"
    ...
    const [showIfIsEmploy, setShowIfIsEmploy] = useState(false)
  ...
  const formik = useFormik({
    ...
    validate: (values) => {
      ...
      if (values.zamestnanie==='Zamestnanec na SK'){
...
const showIfEmploy = () =>{
  ...
  if (formik.values.zamestnanie === "Zamestnanec na SK"||formik.values.zamestnanie === "Zamestnanec v zahraničí"||formik.values.zamestnanie === "Opatrovateľ živnostník EU"||formik.values.zamestnanie === "Zamestnanec na SK"||){
  setShowIfIsEmploy(true)
  showifemploy.style.display = "block";
}
else{
  setShowIfIsEmploy(false);
