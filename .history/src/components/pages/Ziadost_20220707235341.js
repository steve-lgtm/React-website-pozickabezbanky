...
export default function Ziadost() {
  ...
  const handleChange = e => {
    ...
    let urk_sdz = 0.1834;
    if ( e.values.period==1) urk_sdz = 0.3910;
    if ( e.values.period>1 && e.values.period<6 ) urk_sdz = 0.2098; //0.22;
    if ( e.values.period>5 && e.values.period<9 ) urk_sdz = 0.1834;
    console.log(e.values.amount,e.values.period)
    let finalAmount = Math.round(PMT(Math.pow((1+urk_sdz),1/12)-1,(e.values.period*12),-1*e.values.amount)*100) /100;
