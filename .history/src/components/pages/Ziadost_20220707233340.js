...
export default function Ziadost() {
  ...
  const handleChange = e => {
    ...
    if ( values.period>1 && parseFloat(slider2.value)<6 ) urk_sdz = 0.2098; //0.22;
    if ( parseFloat(slider2.value)>5 && parseFloat(slider2.value)<9 ) urk_sdz = 0.1834;
    const finalAmount = Math.round(PMT(Math.pow((1+urk_sdz),1/12)-1,(parseFloat(slider2.value)*12),-1*parseFloat(slider.value))*100) /100;
    setMounthAmount()
  };
  return (
    <>
      ...
      <div className='container-form'>
       ...
       <h2>{values.period}</h2>
        ...
        <label for="mesplat"><h2 style="margin-bottom:0.3em;"> Mesačná splátka: {}€</h2></label>
            </div>
