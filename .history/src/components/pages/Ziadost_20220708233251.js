...
export default function Ziadost() {
 ...
 const changeAmount = () => {
    ...
    let finalAmount = Math.round(PMT(Math.pow((1+urk_sdz),1/12)-1,(formik.values.period*12),-1*formik.values.amount)*100) /100;
    ...
    <>
      ...
      <div className='container-form'>
        <div className='section-name'>Požadovaná suma</div>
            ...
            <input type="range" name="amount" step="100" min="500" max="15000" onChange={(e) =>{formik.handleChange(e); changeAmount()}} value={formik.values.amount} className="slider" id="suma"/>
       ...
       <h2>{formik.values.period}</h2>
            ...
            <input type="range" name="period" step="1" min="1" max="8" onChange={(e) => {formik.handleChange(e); changeAmount()}} value={formik.values.period} className="slider" id="doba"/>
            <div className="pullleft" style={{fontSize: "1rem"}}>
                min.
            </div>
            <div className="pullright" style={{fontSize: "1rem"}}>
